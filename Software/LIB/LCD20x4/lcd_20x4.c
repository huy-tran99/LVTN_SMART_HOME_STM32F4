...
*@retval: none
*/
void LCD_20x4_i2cDeviceCheck(void)
{
...
*@retval: none
*/
void LCD_20x4_Set_Command(uint8_t cmd)
{
...
*@retval: none
*/
void LCD_20x4_Write_Data(uint8_t datax)
{
...
*@retval: none
*/
void LCD_20x4_Clear(void)
{
	LCD_20x4_Set_Command(LCD_CLEAR_DISPLAY);
	HAL_Delay(10);
	str_len = 0;
	line_pos = 1;
}
...
*@retval: none
*/
void LCD_20x4_SetCursor(int line_x, int chr_x)
{
  line_pos = line_x; //hold line position.	
	
	if(((line_x >=1 && line_x <= line_MAX) && (chr_x >=1 && chr_x <= chr_MAX)))
	{		
		LCD_20x4_Set_Command(LCD_SET_DDRAMADDR | Cursor_Data[line_x - 1][chr_x - 1]);		
	}
}
...
*@retval: none
*/
void LCD_20x4_Send_String(char str[], uint8_t mode)
{	 
	...
	{			
		case STR_NOSLIDE:
			...
			while (*str) 
			{
				LCD_20x4_Write_Data (*str++);
				str_len++;
				if(str_len == chr_MAX)
				{
					LCD_20x4_SetCursor(line_pos + 1, 1);
					str_len = 0;
				}			
			}	
		...
		case STR_SLIDE:
		  ...
		  buffer[a]=str++;
			...
			LCD_20x4_SetCursor(line_pos, i[line_pos - 1]);	
				  			
			for(int k = c[line_pos - 1];k < ch_len; k++) 
			LCD_20x4_Write_Data (*buffer[k]);
...
*@retval: none
*/
void LCD_20x4_Print(char const *ch, float value)
{
	char data_ch[BFR_MAX]; //default data size:100.
	
	sprintf(data_ch, ch, value);
	LCD_20x4_Send_String(data_ch, STR_NOSLIDE);	
}
...
*@retval: none
*/
void LCD_20x4_BackLight(uint8_t light_state)
{
	if(light_state == LCD_BL_ON)
	{
    data_BL = LCD_BL_ON;		
		LCD_20x4_Write_Data(0x20); //Empty character
	}
	else if (light_state == LCD_BL_OFF)
	{
		data_BL = LCD_BL_OFF;
		LCD_20x4_Write_Data(0x20);
	}
}
...
*@retval: none
*/
void LCD_20x4_Init(void)
{
	LCD_20x4_Set_Command(LCD_CLEAR_DISPLAY);
	HAL_Delay(1000);
	LCD_20x4_Set_Command(LCD_RETURN_HOME);
	HAL_Delay(5);
	LCD_20x4_Set_Command(LCD_FUNCTION_SET|MODE_4B|MODE_2L|MODE_5X8_DOTS);
	HAL_Delay(5);
	LCD_20x4_Set_Command(LCD_DISPLAY_CONTROL|DISPLAY_ON|CURSOR_OFF|BLINK_OFF);
	HAL_Delay(5);
	LCD_20x4_Set_Command(LCD_SET_DDRAMADDR);
	HAL_Delay(500);
}
